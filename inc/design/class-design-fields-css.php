...
class CxTh_Design_Fields_Css {
	 ...
	 * @return string $css
	 */
	protected function typography_field_css( $vals ) {
		$value = '';
		$vals[1] = cxth_array_value( $vals, 1, 400 );

		if ( $vals[5] ) {
			$this->gf_load[ $vals[5] ][ '400' ] = 1;
			if ( $vals[1] != '400' ) {
		...
		foreach ( $this->font_format as $k => $f )
			$value .= cxth_sprintf_array_val( $f, $vals, $k );

		$value .= $this->font_field_style_css( cxth_array_value( $vals, 3 ) );
